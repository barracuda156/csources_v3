...
{nimErr_ = nimErrorFlag();
	result = (NIM_BOOL)0;
	nimln_(154);	f_1 = 0;
	nimln_(155);	result = open__stdZsyncio_u415(&f_1, filename_p1, ((tyEnum_FileMode__aSVjmZSaHbpn4VsiAwHiBg)0), ((NI)-1));
	if (NIM_UNLIKELY(*nimErr_)) goto BeforeRet_;
	nimln_(156);	{
		if (!result) goto LA3_;
...
{nimErr_ = nimErrorFlag();
	...
	nimln_(47);	{
		NIM_BOOL T3_;
		T3_ = (NIM_BOOL)0;
		T3_ = open__stdZsyncio_u415(&f_1, filename_p1, ((tyEnum_FileMode__aSVjmZSaHbpn4VsiAwHiBg)1), ((NI)-1));
		if (NIM_UNLIKELY(*nimErr_)) goto BeforeRet_;
		if (!T3_) goto LA4_;
